...
def get_reachable_nodes(current_node, min_straight, max_straight):
    ...
    for dy, dx in ALL_DIRECTIONS:
        ...
        direction_min_straight = 1 if (
            dy, dx) == came_from_direction else min_straight
        weight = 0
        if not within_bounds(y + direction_min_straight * dy, x + direction_min_straight * dx):
            continue
        for i in range(1, direction_min_straight):
            weight += heat_loss_map[y + i * dy][x + i * dx]
        for i in range(direction_min_straight, max_straight - direction_straight_count + 1):
            ny, nx = y + i * dy, x + i * dx
...
Graph = dict[int, tuple[int, list[tuple[int, int]]]]


def make_graph(start=(0, 0), target=(len(heat_loss_map) - 1, len(heat_loss_map[-1]) - 1), min_straight=4, max_straight=10):
    def heuristic(y, x):
    ...
    while q:
        ...
        for node, weight in get_reachable_nodes(current_node, min_straight, max_straight):
            if node[0] == target:
                node_id = 0
